...
pub fn serialise_word_hash(
    ...
    fs::write("languages/english/word_list.ron", ron_string)?;
...
pub fn serialise_text_index(
    ...
    fs::write("languages/english/texts_list.ron", ron_string)?;

    Ok(())
}

pub fn deserialise_word_hash() -> HashMap<String, Word> {
    let hashmap: HashMap<String, Word> = match fs::read_to_string("languages/english/word_list.ron")
    {
...
pub fn deserialise_text_index() -> IndexMap<usize, Text> {
    let indexmap: IndexMap<usize, Text> =
        match fs::read_to_string("languages/english/texts_list.ron") {
            Ok(content) => match from_str(&content) {
